...
class ConnectorSettingsFromConfigCustomization(runtimeConfig: RuntimeConfig) : ConfigCustomization() {
    ...
    override fun section(section: ServiceConfig) = writable {
        when (section) {
            is ServiceConfig.Extras -> rustTemplate(
                """
                impl From<&Config> for #{MakeConnectorSettings} {
                    fn from(service_config: &Config) -> #{MakeConnectorSettings} {
                        service_config
                            .timeout_config
                            .as_ref()
                            .map(|timeout_config| #{MakeConnectorSettings} {
                                    http_timeout_config: timeout_config.http_timeouts().clone(),
                                    tcp_timeout_config: timeout_config.tcp_timeouts().clone(),
                                })
                            .unwrap_or_default()
                    }
                }
